...
import { getProductFooterThunk} from "../../store/actions/footerAction";
...
const ProductFooter = () => {
    ...
    const data = useSelector(state => state.footersReducer.productFooter)

    const handleChange = (event, newValue) => {
        setValue(newValue);
    };
    useEffect(() => {
        dispatch(getProductFooterThunk())
    }, [])
    ...
    const handleSubmit = () => {
        axios
            .post(
                `${baseUrl}/productFooter/editImage`,
                {
                    image: thisImg
                },
                {
    ...
    return (
        <Box m={2}>
            <h3 mt={3} mb={3}>Product page footer settings</h3>
            <Box>
                <Box sx={{width: '100%'}}>
