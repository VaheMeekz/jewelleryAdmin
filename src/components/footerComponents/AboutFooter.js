...
const AboutFooter = () => {
    ...
    const data = useSelector(state => state.footersReducer.aboutFooter)

    const handleChange = (event, newValue) => {
        setValue(newValue);
    };
    useEffect(() => {
        dispatch(getAboutFooterThunk())
    }, [])
    ...
    return (
        <Box m={2}>
            <h3 mt={3} mb={3}>About Us page footer settings</h3>
            <Box>
                <Box sx={{width: '100%'}}>
