...
import {getProductBannerThunk} from "../../store/actions/bannersActions";
...
const ProductBanner = () => {
    ...
    const data = useSelector(state => state.bannersReducer.product)
    ...
    useEffect(() => {
        dispatch(getProductBannerThunk())
    }, [])
    ...
    const handleSubmit = () => {
        if (thisImg !== null) {
            axios
                .post(
                    `${baseUrl}/productBanner/editImage`,
                    {
                        image: thisImg,
                    },
                    {
    ...
    return (
        <Box m={2}>
            <h2 mt={3} mb={3}>Product banner</h2>
            <Box>
                {
                    openSection ? (
            ...
            <Box m={2}><Button  color="secondary" color="secondary" variant="contained" onClick={handleChangeTexts}>Submit</Button></Box>
            <Box m={2}>
