...
const DetailBanner = () => {
    ...
    const data = useSelector(state => state.bannersReducer.detail)
    ...
    const handleSubmit = () => {
        if (thisImg !== null) {
            axios
                .post(
                    `${baseUrl}/detailBanner/editImage`,
                    {
                        image: thisImg,
                    },
                    {
    ...
    return (
        <Box m={2}>
            <h2 m={2} mt={3} mb={3}>Product detail page banner</h2>
            <Box>
                {
                    openSection ? (
